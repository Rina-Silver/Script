...
csv_file_out = scr_dir + r'\CSV\farmakopeika.csv'

URL = 'https://farmakopeika.ru/katalog/2613/'
HOST = 'https://farmakopeika.ru'
...
def get_pages_count(html):
    ...
    pagination = pagination_div.find_all('a')
    if pagination:
...
def get_goods(html):
    soup = BeautifulSoup(html, 'html.parser')
    items = soup.find_all('a', class_='catalog-product-items__item-title-link')
    goods = []
    for item in items:
...
def get_content(link):
    ...
    find_div = soup.find('div', class_='itemdetaildescription')
    if find_div:
...
def parse():
    ...
    if html.status_code == 200:
        ...
        for page in range(1, pages_count + 1):
            ...
            html = get_html(URL, params={'nav': 'page-' + str(page)})
