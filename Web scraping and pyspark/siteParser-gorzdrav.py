...
csv_file_out = scr_dir + r'\CSV\gorzdrav.csv'

URL = 'https://gorzdrav.org/category/protivoallergicheskie/'
...
def get_goods(html):
    soup = BeautifulSoup(html, 'html.parser')
    items = soup.find_all('a', class_='c-prod-item__link')
    goods = []
    for item in items:
...
def get_content(link):
    ...
    title = soup.find('h1', class_='b-page-title').get_text(strip=True)
    description = "Без описания"
    find_div = soup.find('div', itemprop='description')
    if find_div:
