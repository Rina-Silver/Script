...
csv_file_out = scr_dir + r'\CSV\planetazdorovo.csv'

URL = 'https://apteka.planetazdorovo.ru/catalog/allergiya/protivoallergicheskie/'
HOST = 'https://apteka.planetazdorovo.ru'
...
def get_goods(html):
    soup = BeautifulSoup(html, 'html.parser')
    items = soup.find_all('a', class_='product-card__image')
    goods = []
    for item in items:
...
def get_content(link):
    ...
    title = soup.find('h1', class_='product-detail__title').get_text(strip=True)
    description = "Без описания"
    find_div = soup.find('div', class_='product-detail-description-content')
    if find_div:
